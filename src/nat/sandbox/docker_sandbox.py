...
class DockerSandbox(BaseSandbox):
    ...
    Note: Requires `docker` package to be installed.
    Install with: pip install nvidia-nat[sandbox-docker]
    """
    ...
    async def start(self) -> None:
        ...
        docker = self._get_docker_module()
        ...
        try:
            ...
            # Add volume mounts if specified (resolve relative paths to absolute)
            if self._volumes:
                resolved_volumes = {}
                for host_path, mount_config in self._volumes.items():
                    # Convert relative paths to absolute paths
                    abs_host_path = os.path.abspath(host_path)
                    resolved_volumes[abs_host_path] = mount_config
                container_config["volumes"] = resolved_volumes
                logger.info(f"Mounting volumes: {list(resolved_volumes.keys())}")
