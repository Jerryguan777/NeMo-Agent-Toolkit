...
This example demonstrates how to use NAT's core sandbox functionality (`nat.sandbox`) to build an agent with code execution capabilities.
...
│                        NAT Core                              │
│  ┌─────────────────┐  ┌─────────────────┐  ┌─────────────┐  │
│  │  nat.sandbox    │  │ nat.sandbox.tools│  │ react_agent │  │
│  │  - DockerSandbox│  │ - execute_shell  │  │ (workflow)  │  │
│  │  - DaytonaSandbox│ │ - execute_python │  │             │  │
│  │                 │  │ - read/write_file│  │             │  │
│  └─────────────────┘  └─────────────────┘  └─────────────┘  │
└─────────────────────────────────────────────────────────────┘
                              │
...
│  ┌─────────────────────────────────────────────────────┐    │
│  │  register.py                                         │    │
│  │  - sandbox_tools: shell, python, file_read/write     │    │
│  │  - web_search: Tavily API (runs on host)             │    │
│  └─────────────────────────────────────────────────────┘    │
└─────────────────────────────────────────────────────────────┘
```
...
# Sandbox is a first-class component in NAT
sandboxes:
  code_sandbox:
    _type: docker
...
workflow:
  _type: sandbox_agent
  llm_name: agent_llm
  sandbox_name: code_sandbox  # Reference to sandbox defined above
  max_iterations: 20
```
...
|--------|---------|-------------|
| `_type` | - | Sandbox type: `docker` or `daytona` |
...
| `volumes` | {} | Host directories to mount |
...
| Option | Default | Description |
|--------|---------|-------------|
| `llm_name` | - | Reference to LLM in `llms` section |
| `sandbox_name` | - | Reference to sandbox in `sandboxes` section |
...
2. **Download GAIA attachments** (required for file-based tasks):
   ```bash
   # Ensure you're in the sandbox_agent directory
   cd examples/advanced_agents/sandbox_agent

   # Download using hf CLI (pip install huggingface_hub[cli])
...
│   ├── __init__.py
│   ├── register.py              # Tool registration (sandbox_tools, web_search)
│   │
│   ├── configs/                 # YAML configurations
│   │   ├── config.yaml          # Basic setup (uses react_agent)
